...
export interface ShopActionRemoveFromCart
  extends Action<'REMOVE_FROM_CART'> {
  productId: string;
}
export interface ShopActionCheckoutSuccess
  extends Action<'CHECKOUT_SUCCESS'> {}
export interface ShopActionCheckoutFailure
  extends Action<'CHECKOUT_FAILURE'> {}
...
export const addToCart: ActionCreator<ThunkResult> = productId => (
  dispatch,
  getState
) => {
...
export const addToCartUnsafe: ActionCreator<
  ShopActionAddToCart
> = productId => {
  return {
    type: ADD_TO_CART,
